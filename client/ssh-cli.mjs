...
export function parseArguments(argv, env = process.env) {
  ...
  const options = { address: env.TAILCAT_ADDR, clientKey: env.TAILCAT_CLIENT_KEY,
    ...
    passphrase: env.SSH_KEY_PASSPHRASE, port: 22, timeout: 1800 };
  const values = { '--user':'username', '-l':'username', '--key':'keyFile', '-i':'keyFile', '--host-key':'hostKey', '--port':'port', '--timeout':'timeout' };
